...
display(count_frames_created("function foo(n) {const y = 1; n + 1;} foo(0); foo(0); foo(0);")); 
//display(count_frames_created("function foo(n) {n + 1;} foo(0); foo(0); foo(0);"));
...
function count_function_objects_created(program_string) {
    ...
    return func_count;//add count_function_objects_created itself
}
...
display(count_function_objects_created("function foo(n) {const y = 1; n + 1;} foo(0); foo(0); foo(0);")); 
...
const primitive_functions = list(
       ...
       list("error",   error            ),
       list("math_abs",math_abs         ),
       list("+",       (x, y) => (is_list(x) && is_list(y)) ? // both x and y are list
                                    append(x, y)  
                                    : (is_number(x) && is_number(y)) ?
                                    x + y
                                    : display("error")), // eg. if x is a list but y is a number
       list("-",       (x, y) => x - y  ),
       list("-unary",   x     =>   - x  ),
       list("*",       (x, y) => x * y  ),
       list("/",       (x, y) => x / y  ),
...
//parse_and_evaluate("1+5;");
parse_and_evaluate(`list(1, 2, 3) + list(4, 5, 6) + list(2, 5) + list(5, 7 ,3);`);
...
// setting up global environment
function map_new(x, y) { // new map that makes use of apply
    function helper(xs) {
        return is_null(xs)
        ? null 
        : pair(apply(x, list(head(xs))), helper(tail(xs)));
    }
    return helper(y);
}
const primitive_functions = list(
